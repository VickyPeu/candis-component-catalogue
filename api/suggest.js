...
const ARCH={
 ...
 bentoxltabs:"gridSection, variant bento_XL_tabs (internally nicknamed the 'big booty bento'): several Bento XL blocks (bentoBoxesXL) presented as TABS — visitors see all topics at a glance via the tab labels and click to switch. Each tab is a full bentoXL with required media (image/video/customHtml), a heading that doubles as the SHORT tab label (the field allows ≤620 but keep it short here), a required paragraph, optional eyebrow (≤270) and up to two links. The counterpart to the sticky scroll: it favors orientation (you see every topic up front, in any order) over a controlled reveal. Recommend up to ~5 tabs — not enforced in Contentful but more breaks the layout on small screens. Use when visitors should choose their topic and order doesn't matter; use the sticky scroll for a guided, sequential narrative. Pillar: Translation of Complexity (organized overview of complex topics) + Standard Setter.",
