...
const ARCH={
 ...
 decision:"1-Column Section showing a decision_overview table. SETUP: create a Table entry, set its variant = decision_overview, fill its content, then add it to the section's decisionOverviewTables field (NOT the media field — media/Table is for the tool_comparison variant). Like the tool comparison but cells hold free TEXT (not checks/crosses) for nuanced, not-black-and-white comparisons. Add several decision_overview tables (each with a category) for the selectable-category version that swaps between them. Leaves the 'right fit' decision to the reader.",
 ...
 steps:"customComponent, variant steps_section: a row of UP TO 5 Bento M cards (the steps field, Contentful-capped at 5) presented as a timeline. Each card has a step label (the timeline marker — e.g. Heute / Tag 14 / Tag 30; the bentoM stepLabel field IS used here, unlike in gridSection), a required image (icon variant does NOT work), a heading (max 80 chars), and a short checklist (the card paragraph). Section eyebrow + heading above; optional CTA button below.",
 ...
 trust:"customComponent, variant trust_section: a short proof/social-proof strip — partner/certification logos, badges, or platform ratings. Items come from trustSectionItems (array of image — the logos/badges) and/or platformRatings (array of platformRatings: image + a star rating number + a url link, e.g. Capterra/Google/App Store). Eyebrow + heading on the left, items on the right. Works with 2 to ~8 items; when they overflow the width it auto-loops as a slider. If every item MUST be visible at first glance, or each logo should deep-link to its own page (e.g. integrations), the Grid Section 'logos_b' variant (Logos / Integrations) is the better choice. Pillar: Trust Anchor (social proof) + Standard Setter (ratings are measurable).",
  llmsection:"customComponent, variant llm_section: an on-page AI chat section (currently on the homepage). Visitors chat with 'Cal', the Candis AI assistant, to get answers to their specific questions instead of hunting the whole site; Cal answers as far as its knowledge allows and PROACTIVELY promotes booking a consultation with sales (via the linked form). Fields: heading (e.g. 'Meet Cal – Candis AI Assistant'), image (Cal's avatar), commonTextSet (intro + suggested prompts), and form (the consultation lead capture). The toggle* fields carry inert placeholder text (Before/After leftovers). A self-serve + lead-gen alternative to browsing; differs from the FAQ Section (static Q&A) — this is a live chat. Pillar: Translation of Complexity (get exactly the info you need, on demand) + Thought Leader (Candis's own AI, in action).",
 videotabs:"tabSection (the video tabs section): 1–4 video tabs (tabs[] = tabContent; each has a tabText label ≤20, a heading, a paragraph ≤200, an eyebrow ≤50, a media video + mobileMedia, optional link). One Boolean, isVideoTabSectionWithoutSound, toggles the whole section between two modes: (a) MUTED screen recordings that autoplay (keep ≤20s; tabs can flow as one continuous storyline 1→2→3) — a quick, real impression of a feature in the product; (b) SOUND videos that must be clicked to play (e.g. short filmed interview impressions of real people) — these need a 16:9 thumbnail per tab. Section-level eyebrow + heading + paragraph. Gives strong proof the product is real (Trust Anchor) AND shows how it behaves (Translation of Complexity). ONE catalog entry covers both modes (one content type + a toggle). Pillar: Trust Anchor + Translation of Complexity.",
  logoslider:"sliderSection with size=logo: a logo slider — a continuous, auto-rolling band of customer/reference logos (content[] = image entries) with an optional short eyebrow + heading (e.g. 'More than 10,000 companies already rely on Candis'). mode is usually continuous_slide; ~6 visible. Social proof through recognizable logos. Editors should pick references that fit the page's target group AND that the visitor will recognize — relevance + recognition is what builds the trust. Pillar: Trust Anchor (social proof) + Standard Setter (market reach). Part of the slider family (testimonials, logo, rating). Differs from the Grid Section logos_b (integration logos that deep-link to subpages) — this one is a rolling trust band.",
  testimonialslider:"sliderSection with size=testimonial: a testimonials slider for social proof. content[] holds testimonials entries; each has a required 1:1 portrait, a required company logo (monochrome vector .eps/.svg), employeeName + employeeTitle, the company name, and a quoteRichText capped at 200 characters. mode controls behaviour: a single testimonial is static; several auto-rotate (slide / continuous_slide). The quote should land ONE benefit that matches the page's messaging — short and punchy. Pillar: Trust Anchor (customer social proof). Part of the slider section family (other sizes: logo, rating).",
  imagesection:"imageSection (no variant): a full-width image band used as a navigation element, an emotional CTA, or a brand-closing section. A required background image + a mobileImage for mobile, an eyebrowHeading + heading, and optionally one or two buttons (button + additionalButton; isFirstButtonPrimary sets which is primary) — or no buttons at all, just a brand moment. Text sits over the image, so keep it legible. Short copy only (eyebrow + heading). Use it to close a page on a brand/emotional note or push to the next step. Differs from the CTA Bento (which carries steps + ratings + testimonials) — this is the simple, image-led version. Pillar depends on the page/context.",
  herofullimage:"heroSection, variant full_image: a bold, full-bleed image hero — a complete background image (media) with eyebrow + heading + a short paragraph + up to two buttons centered over it, in white. A separate portraitModeImage (vertical) is REQUIRED for mobile. The image can show anything, but the centered text must stay legible (designers protect it with a calm/dark area or overlay). Copy is sparse: eyebrow + heading + a ≤3-line paragraph (it's centered, so more is hard to read). No Bento XS, logo slider or trust chips — the image carries the visuals. Best for a visual, brand-moment standout page (campaign, Academy, event). Pillar depends on the page topic.",
  herologoslider:"heroSection, variant customer_logo_slider: the common-hero layout (eyebrow + heading + short paragraph + media + primary/secondary button + trust chips) PLUS an integrated customer logo slider (logoSlider → sliderSection) underneath, e.g. '100+ companies work with Candis' + a row of logos. Defining feature: extreme trust via the logo strip ('look who already works with us'). Media is an image (1:1) or a video (16:9), like the common hero. NO Bento XS here and the paragraph must stay short (~4 lines, no bullets) — there's no room. Use for a trust-first / social-proof-led page (e.g. an industry landing page). Pillar: Trust Anchor (primary — the logo proof) + Translation of Complexity.",
